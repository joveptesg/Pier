...
/// The `http.serversTransports` block declaring `pier-backend-insecure`, or an
/// empty string when no upstream needs it.
///
/// A few images terminate TLS themselves on their own container port with a
/// self-signed cert (KasmVNC on 6901) — their catalog template marks the port
/// `scheme = "https"` and `build_target_url` emits an `https://` upstream.
/// Traefik would reject that cert, so those load balancers reference this
/// transport. It only weakens the Traefik→container hop inside the Docker
/// network; public-facing TLS is unaffected.
fn insecure_transport_section(needed: bool) -> &'static str {
    if needed {
        "  serversTransports:\n    pier-backend-insecure:\n      insecureSkipVerify: true\n"
    } else {
        ""
    }
}

/// The `serversTransport:` line to drop inside a `loadBalancer`, or empty.
fn insecure_transport_ref(needed: bool) -> &'static str {
    if needed {
        "        serversTransport: pier-backend-insecure\n"
    } else {
        ""
    }
}
...
"#
    ...
    let insecure = upstreams.iter().any(|u| u.url.starts_with("https://"));
    let transports_section = insecure_transport_section(insecure);
    let transport_ref = insecure_transport_ref(insecure);
...
{routers}{mw_section}  services:
    pier-{service_id}:
      loadBalancer:
{transport_ref}{sticky_section}        servers:
{servers_yaml}"#
    )
}
...
pub(crate) fn generate_dynamic_config_multi_target(
    ...
    for (group_key, group) in &groups {
        ...
        let upstream = group
            ...
            .unwrap_or_default();
        let transport_ref = insecure_transport_ref(upstream.starts_with("https://"));
        any_insecure |= !transport_ref.is_empty();
...
"#
    ...
    let transports_section = insecure_transport_section(any_insecure);
...
#[cfg(test)]
mod backend_tls_tests {
    use super::{
        generate_dynamic_config_lb, generate_dynamic_config_multi_target, DomainTarget, LbConfig,
        LbUpstream,
    };

    fn target(url: &str) -> DomainTarget {
        DomainTarget {
            domain: "kasm.example.com".to_string(),
            use_tls: true,
            compose_service: None,
    ...
    /// KasmVNC terminates TLS itself on 6901 with a self-signed cert, so the
    /// catalog marks the port `scheme = "https"` and `build_target_url` emits
    /// an `https://` upstream. Traefik must then skip upstream cert checks.
    #[test]
    fn https_upstream_emits_insecure_backend_transport() {
        let yaml =
            generate_dynamic_config_multi_target("svc-1", &[target("https://pier-browser:6901")]);
        assert!(yaml.contains("serversTransports:"), "yaml = {yaml}");
        assert!(yaml.contains("pier-backend-insecure:"), "yaml = {yaml}");
        assert!(yaml.contains("insecureSkipVerify: true"), "yaml = {yaml}");
        assert!(
            yaml.contains("serversTransport: pier-backend-insecure"),
            "yaml = {yaml}"
        );
        assert!(
            yaml.contains("url: \"https://pier-browser:6901\""),
            "yaml = {yaml}"
        );
    }

    /// Every pre-existing service keeps a byte-identical config: no transport
    /// block, no reference.
    #[test]
    fn http_upstream_omits_backend_transport() {
        let yaml =
            generate_dynamic_config_multi_target("svc-1", &[target("http://pier-grafana:3000")]);
        assert!(!yaml.contains("serversTransport"), "yaml = {yaml}");
        assert!(!yaml.contains("insecureSkipVerify"), "yaml = {yaml}");
    }

    /// Scaled services go through the load-balancer generator instead, which
    /// needs the same treatment or every replica would 502.
    #[test]
    fn lb_https_upstreams_emit_insecure_backend_transport() {
        let yaml = generate_dynamic_config_lb(
            "svc-1",
            &[("kasm.example.com".to_string(), true, false)],
            &[
                LbUpstream {
                    url: "https://pier-browser-1:6901".to_string(),
                    weight: 1,
                },
                LbUpstream {
                    url: "https://pier-browser-2:6901".to_string(),
                    weight: 1,
                },
            ],
            &LbConfig::default(),
    ...
    #[test]
    fn lb_http_upstreams_omit_backend_transport() {
        let yaml = generate_dynamic_config_lb(
            "svc-1",
            &[("app.example.com".to_string(), true, false)],
            &[LbUpstream {
                url: "http://pier-app:3000".to_string(),
                weight: 1,
            }],
            &LbConfig::default(),
        );
