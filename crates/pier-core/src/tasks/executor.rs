...
pub async fn start_run(state: &SharedState, spec: StartSpec) -> Result<String, AppError> {
    ...
    let (host, port, agent_token, is_local, kind) =
    ...
    spawn_driver(state.clone(), task_id.clone(), spec.server_id.clone(), agent_run_id);
    Ok(task_id)
}
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            match fetch_agent_snapshot(&host, port, &agent_token, &agent_run_id).await {
                Ok(snap) => {
                    failures = 0;
                    let finished = models::is_terminal(&snap.status)
                        || snap.finished_at_unix_ms.is_some();
                    {
...
async fn call_agent_cancel(
    ...
    let url = format!(
        "http://{host}:{port}/api/v1/agent/shell/{agent_run_id}/cancel"
    );
    let client = reqwest::Client::builder()
        .timeout(Duration::from_secs(5))
        .build()
