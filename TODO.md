...
- [x] **8.1.1** Модуль `src/proxy/mod.rs` — lifecycle Traefik контейнера
...
- [x] **8.1.2** Модуль `src/proxy/config.rs` — генерация конфигов
...
- [x] **8.1.4** Настройки прокси в таблице `settings`
...
- [x] **8.2.1** DB миграция 7 — таблица `domains`
  ```sql
  ...
  ```
- [x] **8.2.2** API `src/api/domains.rs`
...
- [x] **8.2.3** Роуты в `src/api/mod.rs`
...
- [x] **8.3.1** Генерация служебного домена при деплое ресурса
...
- [x] **8.3.2** Определение IP сервера
...
- [x] **8.3.3** Traefik route для служебного домена
  - Автоматически создавать dynamic config
  - SSL через Let's Encrypt (sslip.io поддерживает ACME HTTP-01)
- [x] **8.3.4** UI: показывать служебный домен на странице ресурса
...
- [x] **8.4.1** Background SSL monitor (`src/proxy/ssl_monitor.rs`) — парсит acme.json каждые 15 мин, обновляет ssl_status и ssl_expires_at
...
- [x] **8.5.1** Страница `assets/templates/proxy/settings.html`
...
- [x] **8.5.2** Страница `assets/templates/domains/list.html`
  - Таблица всех доменов: domain, service, SSL status (🔒/⏳/❌), expires
  - Фильтр: все / auto-generated / custom
  - Добавить домен (модалка)
- [x] **8.5.3** Секция доменов на `resources/detail.html`
  - Список привязанных доменов (служебный + кастомные)
  - SSL badge (зелёный/жёлтый/красный)
  - Кнопка "Add Custom Domain"
- [x] **8.5.4** Сайдбар — добавить "Domains" пункт в `base.html`

### 8.6 Компиляция и тест

- [x] **8.6.1** `cargo build --release`
- [x] **8.6.2** `docker compose build && docker compose up -d`
- [x] **8.6.3** Тест: Enable proxy → deploy PostgreSQL → служебный домен появился → открывается в браузере → SSL active
- [x] **8.6.4** Тест: Добавить кастомный домен → Traefik подхватил → SSL получен
...
- [x] **9.1.1** Добавить в workspace Cargo.toml:
  ```toml
  hmac = "0.12"
  sha2 = "0.10"
  hex = "0.4"
...
- [x] **9.2.1** Расширить таблицу `services`:
  ```sql
...
- [x] **9.3.1** `src/api/webhooks.rs` — GitHub webhook
...
- [x] **9.3.2** `src/api/webhooks.rs` — GitLab webhook
...
- [x] **9.4.1** `src/deploy/mod.rs` — оркестратор
...
- [x] **9.4.2** `src/deploy/build.rs` — стратегии билда
...
- [x] **9.4.3** `src/deploy/rollback.rs` — откат
...
- [x] **9.5.1** Git config endpoints в `src/api/resources.rs`:
...
- [x] **9.6.1** Git config секция на `resources/detail.html`
  - Repo URL, branch, build strategy (dropdown)
  - Webhook URL (auto-generated, с кнопкой копирования)
  - "Deploy Now" кнопка
- [x] **9.6.2** Deployments вкладка на `resources/detail.html`
...
- [x] **9.6.3** Live build log viewer
  - SSE подключение при активном билде
  - Auto-scroll, monospace шрифт, зелёный/красный по статусу

### 9.7 Компиляция и тест

- [x] **9.7.1** `cargo build --release`
- [x] **9.7.2** Тест: настроить git source → push в репо → webhook → auto-deploy → success
- [x] **9.7.3** Тест: ручной deploy → build log в реальном времени
- [x] **9.7.4** Тест: rollback → предыдущая версия восстановлена
...
- [x] **10.1.1** Расширить таблицы (миграция 20):
  ```sql
...
- [x] **10.2.1** Helper `get_server_info()` в `src/api/servers.rs`
  ```rust
  ...
  ```
- [x] **10.2.2** Remote deploy через agent proxy endpoints
...
- [x] **10.2.3** `server_id` field в CreateResourceRequest + INSERT
  - Резолвить `server_id` → `ServerInfo`
  - Передавать в `deploy_stack`

### 10.3 Agent proxy

- [x] **10.3.1** Agent proxy в `src/api/servers.rs`
  - `GET /api/v1/servers/{id}/containers` → проксирует к агенту или Bollard локально
  - `POST /api/v1/servers/{id}/deploy` → проксирует к агенту `/api/v1/agent/deploy`
  - `POST /api/v1/servers/{id}/stop` → проксирует к агенту `/api/v1/agent/stop`

### 10.4 Agent install script

- [x] **10.4.1** `GET /api/v1/servers/install-script`
...
- [x] **10.5.1** Server detail page `assets/templates/servers/detail.html`
...
- [x] **10.6.1** `cargo build --release`
- [x] **10.6.2** Тест: деплой ресурса с `server_id` = remote → compose отправлен агенту → контейнер запущен
- [x] **10.6.3** Тест: server detail → метрики обновляются в реальном времени
...
- [x] **11.1.1** Проверка обновлений (реализовано в `src/api/system.rs`)
  ...
  - Если есть новая версия → вернуть info в API response
- [x] **11.1.2** Применение обновления (в `src/api/system.rs::update_now`)
...
- [x] **11.1.3** API endpoints:
  - `GET /api/v1/system/update-check` — текущая версия + доступное обновление ✅
  - `POST /api/v1/system/update` — применить обновление ✅
  - `GET /api/v1/system/update-settings` — настройки обновлений ✅
  - `PUT /api/v1/system/update-settings` — режим (auto/notify/manual), auto_check ✅
- [x] **11.1.4** UI: страница /updates + ссылка в сайдбаре
...
- [x] **11.2.1** DB: Migration 28 в `src/db/schema.rs` — `services.replicas/lb_strategy/lb_sticky_cookie`
      + новая таблица `service_replicas (service_id, server_id, replica_idx, host_port, weight, status)` с backfill.
- [x] **11.2.2** Scale up/down ресурса — `POST /api/v1/resources/{id}/load-balance`
      (cluster-only `/scale` не трогали). Compose: N service-записей `{id}_1..N` с уникальными host-портами
      (не `deploy.replicas` — вне Swarm игнорируется). Traefik dynamic config: список `servers`.
- [x] **11.2.3** Стратегии — round-robin (default) / weighted (per-upstream `weight`) / sticky (cookie).
- [x] **11.2.4** Multi-server — `distribution: [{server_id, replicas, weight}]`. Remote деплой через
      агентский `POST /api/v1/agent/deploy`; remote-compose биндит `0.0.0.0`; Traefik использует
      `http://{server.host}:{host_port}` для удалённых реплик и Docker DNS `http://pier-{name}-{idx}`
      для локальных.
- [x] **11.2.5** UI — секция «Scale & Load Balancer» на `resources/detail.html` (после Cluster Nodes):
      слайдер реплик 1–10, dropdown стратегии, per-server таблица размещения, sticky-cookie input.
- [ ] **v2 follow-ups** — named volumes с `shared_readonly`, live-rebalance при отвале remote-сервера,
      TLS между Traefik и upstream.
...
- [x] Таблица `networks` (id, name, driver, is_default, created_at) — миграция 11
- [x] `pier-net` — встроенная сеть, is_default=1, нельзя удалить
- [x] Колонка `network_id` в `services` (FK → networks, default → pier-net)

### 12.2 API

- [x] `GET /api/v1/networks` — список сетей
- [x] `POST /api/v1/networks` — создать сеть (создаёт Docker network)
- [x] `DELETE /api/v1/networks/{id}` — удалить (если нет сервисов)

### 12.3 Compose генерация

- [x] Добавить `networks:` секцию в compose YAML с выбранной сетью
- [x] При смене сети — regenerate compose + redeploy

### 12.4 UI

- [x] Settings → Networks — CRUD сетей (`templates/networks/list.html`)
- [x] При создании ресурса — dropdown "Network" (как Project)
- [x] На странице ресурса — текущая сеть + возможность сменить
...
| Метрика | План | Факт |
|---------|------|------|
| Новых Rust модулей | ~20 | ~15 реализовано |
| Новых HTML шаблонов | ~10 | ~8 реализовано |
| Новых DB миграций | 4 (7–10) | 13 (7–19) |
| Новых API эндпоинтов | ~35 | 100+ |
| Новых зависимостей | hmac, sha2, hex | hmac, sha2, hex + aws-sdk-s3, cron, gix и др. |

## Прогресс

| Phase | Статус | Прогресс |
|-------|--------|----------|
| 8 — Reverse Proxy + Домены + SSL | **DONE** | 100% |
| 9 — Git Webhooks + Auto-Deploy | **DONE** | 100% |
| 10 — Multi-Server | **DONE** | 100% |
| 11 — Auto-Update + LB + RBAC + Alerts | **IN PROGRESS** | ~10% |
| 12 — Docker Networks | **DONE** | 100% |

---

## Уроки из инцидента с шифрованием env_json (2026-04-14 → 2026-04-18)

Коммит `fda330e` ввёл AES-256 для `env_json` и замаскировал поле в `/resources/:id`,
но не учёл несколько вещей, из-за которых production сломался в 4 проектах.
Фикс: `4e008d6`, `6a929ea`, `4fadfa2`, `4ae47c1`. Чтобы не наступить снова при
RBAC / alerts / любой другой security-фиче:

1. **Ключи шифрования кешировать на процесс (`OnceLock`/`LazyLock`).**
   Иначе каждый encrypt/decrypt может получить свой ключ, если source ключа
   не идемпотентен (env var может отсутствовать, файл может быть нечитаем).

2. **Не доверять `/opt/pier/.env` как writable из рантайма.** `pier.service`
   задаёт `ReadWritePaths=/opt/pier/data /opt/pier/.docker /tmp` — всё
   остальное readonly. Секреты пишем либо из `install.sh` (под root, до
   старта systemd), либо в `{PIER_DATA_DIR}/.*` (всегда writable).

3. **Fail-loud при невозможности сохранить ключ.** Silent fallback на
   свежий random — worst case: данные корраптятся и восстановить их
   можно только если старые ключи случайно попали в journal.

4. **Startup canary-чек (`encrypt("test") → decrypt → assert_eq!`).**
   Если ключ сломан, лучше уронить бинарник на старте, чем тихо портить
   пользовательские данные.

5. **Единая точка encrypt/decrypt (`crypto::encrypt_env_json`/`decrypt_env_json`).**
   Любое место, читающее или пишущее шифруемую колонку, должно ходить
   через helper. В этом инциденте 7 (!) мест бипассили crypto — каждое
   требовало отдельного фикса.

6. **Менять contract API и UI в одном PR.** Коммит `fda330e` замаскировал
   `env_json: null` в `/resources/:id`, но фронт продолжил читать из этого
   поля. Если секьюрити-change ломает контракт — тест/UI-правка должны
   быть в той же миграции, иначе будет «зашифровано, но невидно».

7. **Recovery-файл для данных, зашифрованных старыми ключами.**
   `install.sh` собирает все PIER_SECRET из journald в
   `.pier-recovery-keys`, бинарник на старте пробует каждый ключ на
   ENC-блобах. AES-GCM authenticated — false positives невозможны.
   Рекомендуется использовать тот же паттерн для любой будущей rotation
   ключей.

8. **`install.sh` — первый класс citizen для secrets.** Нельзя полагаться
   на то, что рантайм сможет записать секрет сам. `install.sh` работает
   под root ДО старта systemd и может писать куда угодно — это правильное
   место для первоначальной генерации и записи ключей.
