...
Pier — это платформа как сервис (PaaS) с открытым исходным кодом для самостоятельного размещения, которая работает как **один бинарный файл на Rust**. Разворачивайте контейнеры, стеки Docker Compose и Git-репозитории с автоматическим SSL, обратным прокси и современной веб-панелью — и всё это при потреблении **20–40 МБ оперативной памяти**.
