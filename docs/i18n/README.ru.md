...
- 🗄 Встроенный **редактор данных** — просмотр таблиц/коллекций и выполнение SQL/Mongo/Redis-запросов прямо из панели (PostgreSQL, MySQL/MariaDB, MongoDB, Redis)
...
**Просматривайте и запрашивайте свои базы данных прямо из панели — без Adminer, без pgweb, без внешних клиентов.** У каждого сервиса БД появляется вкладка **Data**: изучайте схему, листайте строки и выполняйте запросы прямо на месте. Встроено в бинарник, ограничено через RBAC, и каждый запрос фиксируется в аудите.

### Поддерживаемые движки

| Движок | Драйвер | Просмотр | Раннер запросов |
|---|---|---|---|
| **PostgreSQL** (вкл. PostGIS, TimescaleDB) | нативный `sqlx` | схемы · таблицы · представления · структура · строки | произвольный SQL |
| **MySQL / MariaDB** | нативный `sqlx` | базы · таблицы · представления · структура · строки | произвольный SQL |
| **MongoDB** | `mongosh` (docker-exec) | базы · коллекции · документы | скрипты `mongosh` |
| **Redis / Valkey** | нативный `redis` | ключи (SCAN) · значения с учётом типа · TTL | сырые команды |

### Просмотр

- **SQL** — дерево схем/таблиц, структура каждой таблицы (столбцы, типы, nullability, значения по умолчанию, первичные ключи, индексы) и постраничные строки с общим количеством.
- **MongoDB** — дерево «база → коллекция», постраничные документы в формате EJSON.
- **Redis** — браузер ключей на основе `SCAN` с типом каждого ключа, просмотром значения с учётом типа (string / list / set / zset / hash / stream) и TTL; переключение между БД 0–15.

### Запросы

- **SQL Runner** — выполнение любого запроса к PostgreSQL или MySQL/MariaDB. Чтения возвращают таблицу (до 1000 строк), записи — количество затронутых строк. Тайм-аут запроса в 15 секунд не даёт «зависшему» запросу нагрузить базу.
- **Mongo Shell** — выполнение любого скрипта `mongosh` к выбранной базе.
- **Команды Redis** — выполнение любой команды (`GET`, `HGETALL`, `TTL`, …) с чтением ответа в JSON.

### Доступ и аудит

- **Чтение** (просмотр, структура, строки) требует роли `Viewer`; **запись** (любой раннер) — роли `Editor`, права проверяются по каждому ресурсу через RBAC Pier.
- Каждый запуск раннера записывается в таблицу аудита `db_query_log` — кто, что и к какой базе выполнил, со статусом, количеством строк и длительностью.
- Подключения используют учётные данные, расшифрованные из зашифрованного окружения сервиса. Приватные базы доступны через Docker-сеть `pier-net`, поэтому ни один порт не нужно публиковать на хост.
